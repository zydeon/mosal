...
void align(){
	...
	int k, i, j, score, last_match = NEG_INF, r, s, t;
	...
	for( k = 1; k <= K; ++k ){
		...
		for( i = 1; i <= M; ++i ){
			for( j = 1; j <= N; ++j ){
				...
				r = R2[i-1][j-1] + score *(R2[i-1][j-1]!=NEG_INF);
				s = S2[i-1][j-1] + score *(S2[i-1][j-1]!=NEG_INF);
				t = T2[i-1][j-1] + score *(T2[i-1][j-1]!=NEG_INF);
