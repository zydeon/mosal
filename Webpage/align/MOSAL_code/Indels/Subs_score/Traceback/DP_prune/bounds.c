...
void calculate_lower_bound( ){
	...
	// FILE *f = fopen("bounds.txt", "w");
	// for( i = 0; i < BOUNDS_NUM; ++i ){
	// 	fprintf(f, "%d %d\n", LB[i].matches, LB[i].indels );
	// }
	// fclose(f);
...
void prune( Node * previous_state, int i, int j){
	/**
	 * If the score vector following 'previous_state' plus the corresponding upper bound
	...
	int m = previous_state->next->score.matches + U[i][j].matches;
	int d = previous_state->next->score.indels + U[i][j].indels;
