...
void calc_MAX(){
	...
	for( i = 1; i < M+1; ++i ){
		...
		for( j = 1; j < N+1; ++j ){
			...
			L[curr][j]  = lexmaxM3( LS[j], LT[curr][j], L[prev][j-1], score, i, j );
...
void calc_MIN(){
	 ...
	 * 	  - the corresponding substitution score can then be easily determined based on
	 * 	  	all possibilities of the gap position in the smaller sequence
	 */

	VMG min;
	int i, score, gap_size;
	...
	if( min.gaps == 0 ){
		...
		for( i = gap_size; i < strlen(greater_seq); ++i )
			score += get_score2(greater_seq,i,smaller_seq,i-gap_size);
		...
		for( i = 0; i < strlen(smaller_seq); ++i ){
			...
			score += get_score(i,i);
			score -= get_score2(greater_seq,i+gap_size,smaller_seq,i);

			/* update minimum substitution score */
			if( score > min.matches)
				min.matches = score;
		}
	}
	...
	for( m = 0; m < MAX_MID_BOUNDS_NUM; ++m ){
		...
		for( i = 1; i < M+1; ++i ){
			...
			for( j = 1; j < N+1; ++j ){
				...
				M_[curr][j] = scalar_max3( MS[j], MT[curr][j], M_[prev][j-1], subs_score, ws, wg );
...
void calculate_upper_bound( ){
	...
	for( i = M-1; i >= 0; --i )				/* begins in the oposite corner of the table (reversed sequences) */
		for( j = N-1; j >= 0; --j )
			U[i][j] = max3(U[i+1][j+1]+get_score(i,j), U[i+1][j] , U[i][j+1] );
}
...
VMG scalar_max3( VMG S, VMG T, VMG Q, int score, int ws, int wg ){
	VMG result;
...
void prune( Node * previous_state, int i, int j, char dir ){
	...
	m = previous_state->next->score.matches + U[i][j];
	g = previous_state->next->score.gaps + add_gap;
    ...
    if( k < BOUNDS_NUM ){
        if( m == LB[k].matches ){
            if( g > LB[k].gaps ){
		        previous_state->next->next = NULL;
				free(previous_state->next);
				previous_state->next = NULL;
            }        
        }
        else{
            if( g >= LB[k].gaps ){
